...
class CustomerController extends Controller
{
    ...
    {
        // დარწმუნდი, რომ ეს Middleware გაწერილი გაქვს Kernel-ში
    ...
    {
        $cities = City::all(); // ახლა უკვე იპოვის კლასს
    ...
    public function store(Request $request)
{
    $this->validate($request, [
        ...
        'city_id' => 'required|exists:cities,id', // ვამოწმებთ, რომ ეს ID ბაზაში არსებობს
        'address' => 'required|string|max:255',
        'email'   => 'required|email|unique:customers,email',
        'tel'     => 'required|unique:customers,tel',
    ]);

    // რადგან ფორმაშიც city_id გვქვია და ბაზაშიც, 
    // $request->all() პირდაპირ იმუშავებს
    ...
    public function update(Request $request, $id)
{
    $this->validate($request, [
        'name'    => 'required|string|max:255',
        'city_id' => 'required|exists:cities,id', // 'city'-ს ნაცვლად ჩაწერე 'city_id'
        'address' => 'required|string|max:255',
    ...
    // რადგან ფორმაშიც city_id გვიწერია და ბაზაშიც, 
    // update($request->all()) პირდაპირ იმუშავებს
...
public function apiCustomers()
{
    ...
    $data = $customers->map(function($item) {
        return [
            ...
            'email'   => $item->email,
            'phone'   => $item->tel, // DataTable-ში data: 'phone' გიწერია, ამიტომ აქ 'phone' დაარქვი
            'action'  => '
